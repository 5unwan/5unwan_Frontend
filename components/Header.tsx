...
export default function Header() {
  ...
  const handleSpread = (event: React.MouseEvent) => {
    ...
    <div className="h-[74px] max-h-[74px] w-full flex-col">
      <div className="flex h-[74px] w-full items-center justify-between">
        <div className="relative left-[19px] text-[20px]">
          <div className="flex items-center text-[23px]" onClick={handleSpread}>
            {formatDate(date)}
      ...
      <div className="h-[68px] w-full">
        {spread ? (
          ...
          <CalendarWeeks date={date} handleDate={handleDate} today={today} />
        )}
      </div>
    </div>
  );
